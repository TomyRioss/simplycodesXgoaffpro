...
7. **Loop y corte**: sin `--stop-after`, recorre el catálogo de Goaffpro en loop indefinido; con `--stop-after N` corta sola al juntar N completadas. El usuario corta en cualquier momento con **ESC** en la consola, Ctrl+Alt+F12, o el botón Detener del launcher. Al cortar, exporta `export.csv`.
...
3. `python main.py` (o `python main.py --stop-after N`) — el script encuentra la ventana de Chrome ya abierta y opera ahí directo (Windows UI Automation). Corre en loop indefinido hasta que apretás **ESC** en la consola; al cortar exporta `export.csv`.
